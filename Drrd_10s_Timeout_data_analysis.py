...
OUTPUT_PATH = os.path.realpath('../../output/tmp/AY+AZ+BB/AY4+5_AZ4_BB5/')+'/'
PREFIX = 'DRRD 10s with Timeout'
PREFIXES = ['AY4+5', 'AZ4+BB5']
LAST_SESSION = 6
list_data = []
...
def plot_average_long_responses(df, above_time= 5, above_sess= 1):
    ...
    sns.boxplot(x='session',y= 'duration', hue= 'group', hue_order=('5s','10s','20s'), data= dfabove)
...
def main():
    ...
    check_response_distribution_per_group(df,group = '5s')
    
    check_response_distribution_per_group(df,group = '10s')
    
    check_response_distribution_per_group(df,group = '20s')
