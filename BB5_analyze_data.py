...
def plot_average_long_responses(df, above_time= 5, above_sess= 1):
    ...
    sns.boxplot(x='session',y= 'duration', hue= 'group', hue_order=('5s','10s'), data= dfabove)
...
def check_individual_variability(df, above_time= 5, above_sess= 1):
    ...
    plt.figure()
    sns.boxplot(x='rat',y= 'duration', hue= 'group',
...
def correlation_in_long_responses_old(df):
    ...
    sns.pairplot(dfaux2[['5s','10s']])
    np.corrcoef(dfaux2['5s'],dfaux2['10s'])
    corr, p_value = pearsonr(dfaux2['5s'],dfaux2['10s'])
...
def main():
    ...
    df= pd.read_csv(DATA_PATH + f'{PREFIX}.csv', index_col=0)
    ...
    sns.lmplot(x='session',y='duration',hue='group', markers=['>'],\
                data= df.groupby(by=['rat','session','group']).duration.mean().reset_index())
    # plot the fraction of reinforced trials
    sns.lmplot(x='session',y='reinforced',hue='group', markers=['>'],\
    ...
    # fit double gaussian
    fit_double_gaussian(df.query('session>=1'), title=f'{PREFIX} all rats all sessions')    
