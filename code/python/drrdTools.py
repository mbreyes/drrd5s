...
def group_name(fNAME):
    ...
    # O group(1) pega apenas o que está dentro dos parênteses (.*)
    # O .strip() no final garante que espaços extras no fim da frase também sumam
        group_name = result.group(1).strip()
    
    print(f"The group is: '{group_name}'")
    return(group_name)
...
def eliminate_beginning(M, startEvent = None, trial_begin_code= 1):
    ...
    if startEvent != None:
        ...
        last_occurrence = [M[k] for k in range(len(M)) if M[k][1] == startEvent][0]
...
def exp_dataframe(PREFIX:str, RATS:list, Data_Path:str, Output_Path:str, ALL_SESSIONS:list):
    ...
    df.to_csv(os.path.realpath(f'{Output_Path}')+'/'+f'{PREFIX}_tentative.csv', index=False)
    return

def exp_dataframe_timeout(PREFIX:str, RATS:list, Data_Path:str, Output_Path:str, ALL_SESSIONS:list):
    ...
    df_timeout = df_timeout.astype({'rat':int,'session':int,'reinforced':int})
    df_timeout.to_csv(os.path.realpath(f'{Output_Path}')+'/'+f'{PREFIX}_tentative.csv', index=False)
    return
