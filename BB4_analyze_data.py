...
OUTPUT_PATH = os.path.realpath('../../output/tmp/BB/BB4/')+'/'
LAST_SESSION = 6
ANIMALS = [105,106,108,109,111,113,114,115]
...
def plot_over_sessions(data = df, var='duration_mean', \
                       compl = "(s)", hue = 'session', gtypes=['bar','lm', 'box', 'point']):
    
    for gtype in gtypes:
              
        if gtype == 'bar':
            plt.figure()    
            sns.barplot(x='rat',y=var,hue=hue,data = data)    
        
        elif gtype == 'lm':
            plt.figure()    
            sns.lmplot(x='rat',y=var,hue=hue,\
                            markers=['>','o','+'], data= data)
            
        elif gtype == 'box':
            plt.figure()    
            sns.boxplot(x='rat',y=var,hue=hue,data=data)
            
        elif gtype == 'point':
            plt.figure()    
            sns.set_style("darkgrid", {"grid.color": ".6", "grid.linestyle": ":"})
            sns.pointplot(x='rat', y=var, hue=hue, data=data, dodge = 0.2 )
        ...
        plt.savefig(os.path.realpath(f'{OUTPUT_PATH}/{PREFIX}_{gtype}plot_{var}_per_session.png'))
        
    return

df_rat = data.query('session ==1 or session ==6').copy()

plot_over_sessions(var = 'duration', data = df_rat, hue = 'session', gtypes = ['bar', 'box', 'point'])

       
# Graphics of Probability Density of duration_mean per group

plt.figure()
data_prob = data.query('duration <12')
sns.set_style("darkgrid", {"grid.color": ".6", "grid.linestyle": ":"})
sns.kdeplot(data_prob[data_prob.session ==1], x = "duration", hue = "group", fill=True, 
...
plt.figure()
g = sns.FacetGrid(df, col = "rat", col_wrap=6, height=3)
g.map(sns.scatterplot, "Fraction responses above 5s", "Last Criterion")
plt.savefig(os.path.realpath(f'{OUTPUT_PATH}/{PREFIX}_criterion_progression.png'))
